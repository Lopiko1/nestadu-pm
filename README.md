...
## For John (workspace owner)

**This needs to live on your setup, because it requires an always-open instance.** That's the main reason it's landing with you rather than running on Joab's laptop.

Full detail in **[docs/system-setup.md](docs/system-setup.md)**. Roughly 20 minutes.

1. **Step 1 is already done** — Joab has created the Slack app. You may still need to approve or reinstall it if scopes changed.
2. **Create an Anthropic API key** at https://console.anthropic.com and add a few dollars of credit. This can be the same key you're already using for NestaduPM. It's what tells a real request apart from "thanks, looks good." Expected cost at our volume: **cents per month** (uses Claude Haiku).
3. **Put both keys on the machine that will run it**, and set up the cron job. The machine only needs to be awake when the job fires; if it sleeps, the next run catches up.

Step 7 in the guide is optional — it installs the Claude skill. Steps 1-6 are the whole product.
...
40 tests. Covers window building, the settle delay, deduplication, summary sanitization, verdict parsing, and the checkbox-array bug described below. No network calls or tokens needed — everything is faked.

---

## Security notes

**Keep this repository private.** It documents the internal Slack setup and operational layout. Nothing here is a secret today, but it's a map.
...
**If you change commands or flags, update `docs/SKILL.md` in the same commit** — otherwise Claude gives people instructions for a version that no longer exists. The test suite checks this.
